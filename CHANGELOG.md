...
- 为控制侧 runtime snapshot 增加目标年龄、超声波/深度输入年龄等字段，供 diagnostics 使用。
...
### Verified
- 在虚拟机 `wheeltec@192.168.220.131` 上清理 `smart_follower_msgs / perception / control / bringup` 构建产物后重新执行：
  - `colcon build --packages-select smart_follower_msgs smart_follower_perception smart_follower_control smart_follower_bringup --symlink-install`
  - `colcon test --packages-select smart_follower_msgs smart_follower_perception smart_follower_control smart_follower_bringup --event-handlers console_direct+`
- 验证结果：**37 tests passed, 0 failures**。
...
- `arbiter_node.cpp`、`ultrasonic_range_node.cpp`、`follower_controller_node.cpp`、`obstacle_avoidance_node.cpp` 进一步瘦身，节点类只保留参数/接口/diagnostics glue code。
...
- 在同一环境完成 `colcon test --packages-select smart_follower_control`，控制侧新增测试通过。
...
- `smart_follower_perception` 构建结构调整为可复用核心库 + 可执行文件，便于测试复用实现。
- README 与 CHANGELOG 重写为 UTF-8 文档。

### Verified
- 在虚拟机 `wheeltec@192.168.220.131` 上完成 `colcon build --symlink-install`。
- 在虚拟机 `wheeltec@192.168.220.131` 上完成 `colcon test`。
...
- README 增补方案 B：目标机优先使用 `validate` / `train`，完整 `export` 推荐在本地开发机执行。
...
### Added
- 控制链路四个核心节点补齐运行时参数热更新：`follower_controller_node`、`arbiter_node`、`obstacle_avoidance_node`、`ultrasonic_range_node`。
